...
export class DetailService {
   ...
   post(detail: Detail): Promise<any> {
      return this.http.post(this.url, JSON.stringify(detail), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { return error.json(); });
   }

   put(detail: Detail): Promise<any> {
      return this.http.put(this.url, JSON.stringify(detail), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { return error.json(); });
   }
